...
/* ---------------------------------------------------------
   CAROUSEL PREMIUM DRAG + KEYBOARD SWING (DESKTOP)
--------------------------------------------------------- */
function enableCarouselDragScroll(carousel) {
  let isDown = false;
  let startX = 0;
  let scrollLeft = 0;
  let velocity = 0;
  ...
  // Keyboard left/right premium swing
  carousel.tabIndex = 0;
  ...
  carousel.addEventListener("keydown", (e) => {
    ...
    let step = e.key === "ArrowLeft" ? -60 : 60;
    velocity = -step;
...
/* ---------------------------------------------------------
   SIDEBAR PREMIUM SWING — MOBILE + DESKTOP + KEYBOARD
--------------------------------------------------------- */
...
function startDrag(y) {
  ...
  dragMoved = false;
  dragStartY = y;
  dragScrollStart = sidebar.scrollTop;
  dragLastY = y;

  sidebar.style.cursor = "pointer";

  if (dragMomentum) cancelAnimationFrame(dragMomentum);
}
...
function moveDrag(y) {
  ...
  const walk = dragStartY - y;

  if (Math.abs(walk) > 3) dragMoved = true;

  sidebar.scrollTop = dragScrollStart + walk;

  dragVelocity = y - dragLastY;
  dragLastY = y;
}
...
function endDrag() {
  ...
  function momentum() {
    ...
    sidebar.scrollTop -= dragVelocity;
    dragVelocity *= 0.92;

    dragMomentum = requestAnimationFrame(momentum);
  }
...
sidebar.addEventListener("mouseup", endDrag);
sidebar.addEventListener("mouseleave", endDrag);
...
/* ---------------------------
   KEYBOARD UP/DOWN PREMIUM SWING
---------------------------- */
sidebar.tabIndex = 0;
...
sidebar.addEventListener("keydown", (e) => {
  ...
  let step = e.key === "ArrowUp" ? -50 : 50;
  dragVelocity = -step;

  function momentum() {
    if (Math.abs(dragVelocity) < 0.2) return;
